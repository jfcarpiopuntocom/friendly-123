...
const CANAL = (self.registration && /\/next\/$/.test(self.registration.scope || "")) ? "-next" : "";
const CACHE_LOCAL = CACHE + CANAL;
function esDeMiCanal(nombre) {
  return nombre.startsWith("f123-shell-") && (CANAL ? nombre.endsWith("-next") : !nombre.endsWith("-next"));
}
