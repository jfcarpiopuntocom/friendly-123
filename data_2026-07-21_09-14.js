...
function getActivacion() {
  return db.get("configuracion.activacion").value() || { instanceId: null, email: null, activatedAt: null, instanceKey: null };
}
// GAP DE SEGURIDAD CERRADO (JFC 2026-07-21): antes activarInstancia() solo
// guardaba instanceId/email — ninguna ruta /api/* verificaba nada, así que
// el PIN del dueño (100% client-side) no protegía el backend real en
// absoluto: cualquiera que supiera la URL del servidor podía leer/borrar/
// reemplazar todo el negocio sin PIN. instanceKey es el secreto que ahora
// exige el middleware de server.js en cada ruta (ver ahí). Se genera UNA
// sola vez (idempotente: si ya existía, se conserva) — así reactivar el
// mismo negocio nunca rota la key por accidente y rompe el frontend ya
// configurado.
function activarInstancia({ instanceId, email } = {}) {
  const existente = getActivacion();
  const instanceKey = existente.instanceKey || randomBytes(24).toString("base64url");
  const a = {
    instanceId: instanceId || existente.instanceId || randomUUID(),
