...
(async () => {
  ...
  try {
    ...
    // --- LORD: NO adopta, registra el acceso (invitado/observador) ------------
    ...
    check("LORD: NO adopta la licencia ajena (conserva su identidad James Bond)",
      String(oL.licenseCode || "").replace(/\s+/g, "").toUpperCase() === OWN, oL);
